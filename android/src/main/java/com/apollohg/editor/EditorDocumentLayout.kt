...
internal class EditorDocumentLayout(
    ...
    init {
        ...
        val trailingCodeLines = boxes.filter {
            (
                (it.span as? EditorBlockBoxSpan)?.nodeType == "codeBlock" ||
                    it.span is CodeBlockSpan
                ) &&
        ...
        for (box in boxes) {
            ...
            val lastOffset = if (box.end in
                trailingCodeLines
            ) {
                box.end
            } else {
                maxOf(box.start, box.end - 1)
            }
            closes[paragraphAt(lastOffset)] += box
        }
        val reusable = if (previous != null && previous.paintKey == paintKey &&
            previous.includeFontPadding == includeFontPadding &&
            previous.fragmentSpacingMultiplier == spacingMultiplier &&
            previous.fragmentSpacingAdd == spacingAdd
        ) {
            previous.fragments.groupBy {
                it.key
        ...
        for ((index, paragraph) in paragraphs.withIndex()) {
            ...
            if (local.isEmpty() && paragraph.start in trailingCodeLines) {
                content.getSpans(
                    ...
                    .filter {
                        content.getSpanStart(it) < paragraph.start &&
                            content.getSpanEnd(it) >= paragraph.start
                    }
            ...
            local.setSpan(
                EditorOwnedBlockGeometrySpan,
                0,
                local.length,
                Spanned.SPAN_INCLUSIVE_INCLUSIVE
            )
            val requestedAlignment = local.getSpans(
                0,
                local.length,
                EditorParagraphAlignmentSpan::class.java
            ).lastOrNull()?.value
            ...
            if (local.isEmpty()) {
                local.getSpans(0, 0, MetricAffectingSpan::class.java).forEach {
                    it.updateMeasureState(fragmentPaint)
                }
            }
            val shaped =
                cached?.layout
            ...
            val emptyExtra = (
                (emptyStyles.maxOfOrNull { it.lineHeightPx ?: 0 } ?: 0) -
                    shaped.height
                ).coerceAtLeast(0)
            val topTrim =
                (if (includeFontPadding && index > 0) -shaped.topPadding else 0) - emptyExtra / 2
            val bottomTrim =
                (
                    if (includeFontPadding &&
                        index < paragraphs.lastIndex
                    ) {
                        shaped.bottomPadding
                    } else {
                        0
                    }
                    ) -
                    (emptyExtra - emptyExtra / 2)
            val lastLine = shaped.lineCount - 1
            val naturalHeight =
                shaped.getLineDescent(lastLine) - bottomTrim - shaped.getLineAscent(lastLine) -
                    if (lastLine == 0) topTrim else 0
            val afterSpacing = if (index <
                paragraphs.lastIndex
            ) {
                kotlin.math.round(naturalHeight * (spacingMultiplier - 1) + spacingAdd).toInt()
            } else {
                0
            }
            val fragment =
                Fragment(
                    ...
                    y - topTrim, builtLines.size, shaped, key, topTrim, bottomTrim, afterSpacing
                )
            built += fragment
    ...
    internal fun emptyLinePaint(offset: Int): TextPaint? =
        lines[getLineForOffset(offset)].fragment.let {
            if (it.start == it.end) it.layout.paint else null
        }

    internal fun contentLeft(line: Int): Float = lines[line].let {
        it.fragment.x +
            it.fragment.layout.getParagraphLeft(it.local)
    }
    internal fun contentRight(line: Int): Float = lines[line].let {
        it.fragment.x +
            it.fragment.layout.getParagraphRight(it.local)
    }
    internal fun textLineTop(line: Int): Int = lines[line].let {
        it.fragment.y +
            it.fragment.layout.getLineTop(it.local) +
            if (it.local == 0) it.fragment.topTrim else 0
    }
    internal fun textLineBottom(line: Int): Int = lines[line].let {
        it.fragment.y +
            it.fragment.layout.getLineBottom(it.local) +
            if (it.local ==
                it.fragment.layout.lineCount - 1
            ) {
                it.fragment.afterSpacing - it.fragment.bottomTrim
            } else {
                0
            }
    }
    internal fun boxBounds(span: EditorBlockBoxSpan): RectF? =
        boxes.firstOrNull { it.span === span }?.bounds?.let(::RectF)
    internal fun imageBounds(span: BlockImageSpan): RectF? {
        ...
        return span.boxRect(
            minOf(getPrimaryHorizontal(start), getPrimaryHorizontal(end)),
            getLineBaseline(getLineForOffset(start)).toFloat()
        )
    }
    override fun getHeight(): Int = documentHeight
    override fun getLineCount(): Int = lines.size
    override fun getLineTop(line: Int): Int = if (line ==
        lines.size
    ) {
        documentHeight
    } else {
    ...
    override fun getLineDescent(line: Int): Int = lines[line].let {
        getLineTop(line + 1) -
            it.fragment.y -
            it.fragment.layout.getLineBaseline(it.local)
    }
    override fun getLineStart(line: Int): Int = if (line ==
        lines.size
    ) {
        content.length
    } else {
        lines[line].let {
            it.fragment.start +
                it.fragment.layout.getLineStart(it.local)
        }
    }
    override fun getParagraphDirection(line: Int): Int =
        lines[line].let { it.fragment.layout.getParagraphDirection(it.local) }
    override fun getLineContainsTab(line: Int): Boolean =
        lines[line].let { it.fragment.layout.getLineContainsTab(it.local) }
    override fun getLineDirections(line: Int): Directions =
        lines[line].let { it.fragment.layout.getLineDirections(it.local) }
    ...
    override fun getLineLeft(line: Int): Float = lines[line].let {
        it.fragment.x +
            it.fragment.layout.getLineLeft(it.local)
    }
    override fun getLineRight(line: Int): Float = lines[line].let {
        it.fragment.x +
            it.fragment.layout.getLineRight(it.local)
    }
    override fun getLineMax(line: Int): Float =
        lines[line].let { it.fragment.layout.getLineMax(it.local) }
    override fun getLineWidth(line: Int): Float =
        lines[line].let { it.fragment.layout.getLineWidth(it.local) }
    override fun getLineVisibleEnd(line: Int): Int = lines[line].let {
        it.fragment.start +
            it.fragment.layout.getLineVisibleEnd(it.local)
    }
    ...
    override fun getLineBounds(line: Int, bounds: Rect): Int {
        bounds.set(
            contentLeft(line).toInt(),
            textLineTop(line),
            contentRight(line).toInt(),
            textLineBottom(line)
        )
    ...
    override fun getLineForVertical(vertical: Int): Int {
        ...
        return paintedLines.firstOrNull {
            vertical >= textLineTop(it) &&
                vertical < textLineBottom(it)
        }
            ?: paintedLines.minBy {
                maxOf(textLineTop(it) - vertical, vertical - textLineBottom(it), 0)
            }
    }
    override fun getLineForOffset(offset: Int): Int = findLine {
        getLineStart(it) <=
            offset.coerceIn(0, content.length)
    }
    ...
    private fun horizontal(offset: Int, secondary: Boolean): Float {
        ...
        return fragment.x +
            if (secondary) {
                fragment.layout.getSecondaryHorizontal(
                    local
                )
            } else {
                fragment.layout.getPrimaryHorizontal(local)
            }
    }
    override fun getOffsetForHorizontal(line: Int, horiz: Float): Int = lines[line].let {
        it.fragment.start +
            it.fragment.layout.getOffsetForHorizontal(it.local, horiz - it.fragment.x)
    }
    ...
    private fun adjacentOffset(offset: Int, left: Boolean): Int {
        ...
        val local = (offset - fragment.start).coerceIn(0, fragment.end - fragment.start)
        val next = if (left) {
            fragment.layout.getOffsetToLeftOf(
                local
            )
        } else {
            fragment.layout.getOffsetToRightOf(local)
        }
    ...
    override fun getSelectionPath(start: Int, end: Int, dest: Path) {
        ...
        for (fragment in fragments) {
            ...
            for (localLine in fragment.layout.getLineForOffset(
                localFrom
            )..fragment.layout.getLineForOffset(localTo)) {
                ...
                matrix.setRectToRect(
                    RectF(
                        0f,
                        fragment.layout.getLineTop(localLine).toFloat(),
                        1f,
                        fragment.layout.getLineBottom(localLine).toFloat()
                    ),
                    RectF(
                        fragment.x,
                        textLineTop(line).toFloat(),
                        fragment.x + 1,
                        textLineBottom(line).toFloat()
                    ),
            ...
            if (to > fragment.end && fragment.end < content.length) {
                ...
                val edge = if (getParagraphDirection(line) <
                    0
                ) {
                    contentLeft(line)
                } else {
        ...
        for (fragment in fragments) {
            ...
            val localHighlight = highlight?.let {
                Path(it).apply {
                    ...
                    bounds.bottom =
                        (fragment.layout.height - fragment.bottomTrim + fragment.afterSpacing)
                            .toFloat()
...
internal fun Layout.editorTextLineTop(line: Int): Int =
    (this as? EditorDocumentLayout)?.textLineTop(line) ?: getLineTop(line)
internal fun Layout.editorTextLineBottom(line: Int): Int =
    (this as? EditorDocumentLayout)?.textLineBottom(line) ?: getLineBottom(line)
